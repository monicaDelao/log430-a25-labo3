...
def test_stock_flow(client):
    ...
    user_data = {'name': f'Test User {timestamp}', 'email': f'test{timestamp}@example.com'}
    ...
    product_data = {'name': f'Test Product {timestamp}', 'sku': f'TEST{timestamp}', 'price': 15.99}
    ...
    assert stock_check_response.status_code == 201  # Note: L'API retourne 201 (devrait être 200)
    ...
    assert stock_after_order_response.status_code == 201  # Note: L'API retourne 201 (devrait être 200)
    ...
    assert stock_after_delete_response.status_code == 201  # Note: L'API retourne 201 (devrait être 200)
