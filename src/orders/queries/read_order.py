...
def get_highest_spending_users_mysql():
    ...
    try:
        results = (
            session.query(
                Order.user_id, func.sum(Order.total_amount).label("total_expense")
            )
...
def get_best_selling_products_mysql():
    ...
    try:
        ...
        for order_item in order_items:
            result.append(
                {"product_id": order_item[0], "quantity": round(order_item[1], 2)}
            )
...
def get_highest_spending_users_redis():
    ...
    highest_spending_users = sorted(spending.items(), key=lambda x: x[1], reverse=True)[
        :limit
    ]
    for user in highest_spending_users:
        result.append({"user_id": user[0], "total_expense": round(user[1], 2)})
...
def get_best_selling_products_redis():
    ...
    best_selling = sorted(product_sales.items(), key=lambda x: x[1], reverse=True)[
        :limit
    ]
    for product in best_selling:
        result.append({"product_id": product[0], "quantity_sold": product[1]})
